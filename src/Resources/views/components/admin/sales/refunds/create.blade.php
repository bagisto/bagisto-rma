...
@pushOnce('scripts')
                        ...
                        <div class="grid p-4 !pt-0">
                            <div class="grid">
                                ...
                                @foreach ($order->items as $item)
                                    ...
                                    @php
                                        $canRefundQty = app('\Webkul\RMA\Helpers\Helper')->getRMAStatus($item->id);
                                    @endphp
                                    ...
                                    @if ($item->qty_to_refund)
                                        ...
                                        <div class="justify-between gap-2.5 border-b border-slate-300 pb-4 dark:border-gray-800">
                                                ...
                                                <div>
                                                    ...
                                                    <x-admin::form.control-group class="!mb-0">
                                                        <x-admin::form.control-group.control
                                                            ...
                                                            :rules="'required|numeric|min_value:0|max_value:' . $canRefundQty['qty'] - $item->qty_refunded"
                                                            v-model="refund.items[{{ $item->id }}]"
                                                            :label="trans('admin::app.sales.refunds.create.qty-to-refund')"
                                                            :readonly="$canRefundQty['qty'] - $item->qty_refunded"
                                                        />
                                            ...
                                            @if ($canRefundQty['message']) 
                                                <p class="mt-1 text-xs italic text-red-600">{{ $canRefundQty['message'] }}</p>
                                            @endif
                                        </div>
                                    @endif
                                @endforeach
    ...
    <script type="module">
        app.component('v-create-refund', {
            ...
            mounted() {
                @foreach ($order->items as $item)
                    this.refund.items[{{$item->id}}] = {{ app('\Webkul\RMA\Helpers\Helper')->getRMAStatus($item->id)['qty'] - $item->qty_refunded }};
                @endforeach
